...
export default class extends Controller {
  ...
  mapClicked(event) {
    if (this.wasPanning) { this.wasPanning = false; return; }
    ...
    if (event.target.closest("[data-sea-chart-target='routeEditorPanel']")) return;
  ...
  pointerPreview(event) {
    if (!this.routeSourceId || !this.routeSourceNode || !this.hasMapAreaTarget) return
    ...
    const hoveredNode = hoveredElement?.closest?.("[data-sea-chart-target='node']")

    if (hoveredNode && String(hoveredNode.dataset.portId) !== String(this.routeSourceId)) {
  ...
  startRoute(event) {
    ...
    this.routeSourceNode = event.currentTarget.closest("[data-sea-chart-target='node']")
    this.routeHoveredTargetNode = null
    const mapRect = this.mapAreaTarget.getBoundingClientRect()
  ...
  routeLineEntered(event) {
    if (!this.editModeValue || this.routeSourceId || this.routeControlsPinned) return
  ...
  routeLineLeft(event) {
    ...
    if (event.relatedTarget?.closest?.("[data-sea-chart-target='routeControls']")) return
    if (event.relatedTarget?.closest?.("[data-sea-chart-target='routeEditorPanel']")) return
  ...
  toggleRouteEditor(event) {
    ...
    if (!this.routeEditorPanelTarget.hidden) this.showRouteEditor(this.selectedRouteId)
  }
  ...
  async setSelectedRouteDirection(event) {
    ...
    const response = await fetch(`/creator/sea_routes/${this.selectedRouteId}/set_direction`, {
      method: "PATCH",
      headers: {
  ...
  async deleteSelectedRoute(event) {
    ...
    const confirmed = window.confirm("Sei sicuro di voler eliminare questa rotta?")
    if (!confirmed) return

    const csrfTokenMeta = document.querySelector('meta[name="csrf-token"]')
    if (!csrfTokenMeta) return

    const response = await fetch(`/creator/sea_routes/${this.selectedRouteId}`, {
      method: "DELETE",
      headers: {
        "X-CSRF-Token": csrfTokenMeta.content,
        "Accept": "application/json"
  ...
  syncEditMode() {
    if (this.hasModifyButtonTarget) {
      this.modifyButtonTarget.textContent = this.editModeValue ? "✕ Annulla" : "✎ Modifica"
      this.modifyButtonTarget.title = this.editModeValue ? "Chiudi modifica" : "Modalita modifica"
      this.modifyButtonTarget.setAttribute("aria-label", this.editModeValue ? "Chiudi modifica" : "Modalita modifica")
      this.modifyButtonTarget.style.background = this.editModeValue ? "#0f766e" : ""
      this.modifyButtonTarget.style.color = this.editModeValue ? "#ffffff" : ""
      this.modifyButtonTarget.style.borderColor = this.editModeValue ? "#0f766e" : ""
    }
  ...
  syncFullscreenButton() {
    ...
    if (!document.fullscreenEnabled) {
      ...
      this.fullscreenButtonTarget.setAttribute("aria-label", "Full screen non supportato");
      return;
    }
    ...
    this.fullscreenButtonTarget.setAttribute("aria-label",
      document.fullscreenElement === this.mapAreaTarget ? "Esci da full screen" : "Full screen"
    );
  }
  ...
  updatePanTransform() {
    ...
    if (this.hasRadarDotsTarget && this.hasRadarViewportTarget && this.hasMapAreaTarget) {
      const scale = 20;
      const mapRect = this.mapAreaTarget.getBoundingClientRect();
      ...
      this.radarDotsTarget.style.setProperty('--radar-pan-x', `${this.panX / scale}px`);
      this.radarDotsTarget.style.setProperty('--radar-pan-y', `${this.panY / scale}px`);
    }
  }

  panStart(event) {
    if (this.addModeValue || this.editModeValue || this.routeSourceId) return;
    if (event.target.closest('[data-sea-chart-target="node"]') || event.target.closest('.sea-island')) return;
    if (event.target.closest('.hud-element') || event.target.closest('[data-sea-chart-target="routeControls"]')) return;
    if (event.target.closest('[data-sea-chart-target="radar"]') || event.target.closest('[data-action*="radarClick"]') || event.target.closest('[data-sea-chart-target="editTools"]')) return;

    this.isPanning = true;
    this.wasPanning = false;
    this.panStartX = event.clientX || (event.touches && event.touches[0].clientX);
    this.panStartY = event.clientY || (event.touches && event.touches[0].clientY);
  ...
  drag(event) {
    if (this.isPanning) {
      ...
      if (Math.abs(clientX - this.panStartX) > 2 || Math.abs(clientY - this.panStartY) > 2) {
  ...
  updateRoutingLines(portId, newX, newY) {
    ...
    if (this.selectedRouteId && updatedRouteIds.has(this.selectedRouteId)) this.showRouteControls(this.selectedRouteId)
  }

  applySeaRouteState(routeId, seaRoute) {
    if (!seaRoute) return

    const routeElements = this.element.querySelectorAll(`[data-route-id='${routeId}']`)

    routeElements.forEach((element) => {
      if (seaRoute.source_port_id != null) element.dataset.sourceId = String(seaRoute.source_port_id)
      if (seaRoute.target_port_id != null) element.dataset.targetId = String(seaRoute.target_port_id)
      if (seaRoute.source_port_name != null) element.dataset.sourceName = seaRoute.source_port_name
      if (seaRoute.target_port_name != null) element.dataset.targetName = seaRoute.target_port_name
      if (seaRoute.bidirectional != null) element.dataset.bidirectional = String(seaRoute.bidirectional)
    })
  ...
  syncRouteAppearance(routeId) {
    const visibleLine = this.element.querySelector(`[data-route-role='visible'][data-route-id='${routeId}']`)
    if (!visibleLine) return

    const bidirectional = visibleLine.dataset.bidirectional === "true"
    const arrowElements = this.element.querySelectorAll(`[data-route-arrow-index][data-route-id='${routeId}']`)

    visibleLine.setAttribute("stroke-dasharray", bidirectional ? "10,7" : "6,14")
    ...
    const fractions = Array.from({ length: arrowCount }, (_, index) => (index + 1) / (arrowCount + 1))
  ...
  hideRouteControls() {
    ...
    if (this.hasRouteEditorPanelTarget) this.routeEditorPanelTarget.hidden = true
  }

  showRouteEditor(routeId) {
    if (!this.hasRouteEditorPanelTarget || !this.hasRouteEditorTitleTarget || !this.hasRouteEditorStateTarget || !this.hasRouteEditorCycleTarget) return
  ...
  syncRouteDirectionButton(routeId) {
    if (!this.hasRouteDirectionButtonTarget || !this.hasRouteDirectionIconTarget) return
    ...
    this.routeDirectionButtonTarget.setAttribute("aria-label", this.routeDirectionButtonTarget.title)
  }
  ...
  showRouteDirectionMenu(routeId) {
    ...
    this.routeDirectionOptionTargets.forEach((option) => {
      ...
      option.textContent =
        state === "bidirectional" ? "— Neutra" :
        state === "source_to_target" ? `→ ${sourceName} -> ${targetName}` :
      ...
      option.style.background = active ? "rgba(15, 118, 110, 0.24)" : "transparent"
    })
  }
  ...
  removeRoute(routeId) {
    this.element.querySelectorAll(`[data-route-id='${routeId}']`).forEach((element) => {
  ...
  dragEnd(event) {
    if (this.isPanning) {
        this.isPanning = false;
        this.mapAreaTarget.style.cursor = this.addModeValue ? "crosshair" : "default";
  ...
  syncRoutePreview() {
    ...
    const active = this.editModeValue && !!this.routeSourceId && !!this.routeSourceNode
    ...
    if (this.lastPointerPosition) {
      this.updateRoutePreview(this.lastPointerPosition.x, this.lastPointerPosition.y)
      return
    }
  ...
  updateRoutePreview(x, y) {
    if (!this.hasPreviewLineTarget || !this.hasPreviewMarkerTarget || !this.routeSourceNode) return
  ...
  updateRoutePreviewToNode(node) {
    ...
    const sourcePoint = this.nodeConnectionPoint(this.routeSourceNode, targetCenter.x, targetCenter.y)
    const targetPoint = this.nodeConnectionPoint(node, sourcePoint.x, sourcePoint.y)
  ...
  nodeCenter(node) {
    ...
    return {
      x: Math.round(nodeRect.left - mapRect.left - this.panX + nodeRect.width / 2),
      y: Math.round(nodeRect.top - mapRect.top - this.panY + nodeRect.height / 2)
    }
  }
  ...
  syncRouteCursor() {
    ...
    if (this.routeSourceId) {
      this.mapAreaTarget.style.cursor = this.routeHoveredTargetNode ? "copy" : "crosshair"
  ...
  syncRouteEndpoints(routeId) {
    const routeLines = this.element.querySelectorAll(`[data-route-id='${routeId}']`)
    if (!routeLines.length) return

    const visibleLine = this.element.querySelector(`[data-route-role='visible'][data-route-id='${routeId}']`)
    if (!visibleLine) return

    const sourceNode = this.nodeTargets.find((node) => node.dataset.portId === visibleLine.dataset.sourceId)
    const targetNode = this.nodeTargets.find((node) => node.dataset.portId === visibleLine.dataset.targetId)
